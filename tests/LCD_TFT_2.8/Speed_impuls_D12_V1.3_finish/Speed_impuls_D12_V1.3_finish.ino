...
void loop(void) {
     if((digitalRead(impuls) == HIGH) && (sign == 0) ){
      ...
      previousMillis = millis(); 
        //kmh = i/k;
        kmh = i*3600/k; //-разкомментировать, если показания не верны 
        long X = kmh;
...
unsigned long Text(long X) {
  ...
  */
  ...
  String speedo2;                                         //декларируем строку из 2 символов
  String speedo3;                                         //декларируем строку из 3 символов

  if(speedo.length() == 2){                               //проверяем длину строки в 2 символа
    speedo2 = speedo.substring(0,1);                      //запоминаем первое число строки
    ...
    speedo3 = speedo.substring(1,2);                      //запоминаем второе число строки
    String predSpeedo3 = predSpeedo.substring(1,2);       //декларируем строку для второго числа из предыдущих показаний
    if(speedo3.toInt() != predSpeedo3.toInt()){           //если они не равны, то:
      tft.fillRect(80, 80, 55, 80, BLACK);
    }
  }
